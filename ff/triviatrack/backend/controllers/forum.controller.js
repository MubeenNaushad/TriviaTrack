...
export const createPost = async (req, res) => {
    try {
        ...
        const newPost = new ForumPost({ title, description, content, author, category });
...
export const getPostbyId = async (req, res) => {
    try {
        ...
        const post = await ForumPost.findById(postId).populate("author", "name photoUrl").populate("category", "name slug description");

        if(!post) {
            return res.status(404).json({ message: "Post not found" });
        }

        const comments = await ForumComment.find({ postId: post._id }).populate("author", "name photoUrl").sort({ createdAt: -1 });
