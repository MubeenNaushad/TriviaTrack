        ...
        <Link to={`course-details/${course._id}`}>
          <Card className="overflow-hidden rounded-lg big-white shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300">
            ...
            <h1 className="hover:underline font-bold text-lg truncate cursor-pointer">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Avatar className="h-8 w-8">
                    <AvatarImage
                      src={course?.creator?.photoUrl ? course?.creator?.photoUrl : "https://github.com/shadcn.png" }
                      alt="@shadcn"
                    />
                    <AvatarFallback>CN</AvatarFallback>
                  </Avatar>
                  <h1 className="font-medium text-sm">{course?.creator?.name ? course?.creator?.name : "TriviaTrack" }</h1>
                </div>
                <Badge
                  className={
              ...
              <div className="text-lg font-medium">{course?.coursePrice ? course?.coursePrice : "1500 PKR" }</div>
            </CardContent>
          </Card>
        </Link>
    </div>
